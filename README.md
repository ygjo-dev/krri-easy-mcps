...
> **현재 상태: bootstrap.** Gateway client 와 agentic_ai client 는 둘 다 **mock** 이다.
> 화면의 MCP 목록 · 상태 · Tool · 실행 결과 · 등록 미리보기는 전부 mock 데이터다.
...
│   │   └── clients/         gateway.py · agentic_ai.py (mock)
│   └── tests/
├── config/
...
설정은 `.env.example` 참고. BFF 는 환경변수를 읽는다.
...
- production technical MCP/tool 정보의 source of truth 는 향후 **Gateway / live MCP `tools/list`** 다.
  `api/app/clients/gateway.py` 의 mock 데이터는 UI 검증용 대표 subset 이고 production catalog 가 아니다.
...
- live Gateway catalog / status / tools/list 연동
